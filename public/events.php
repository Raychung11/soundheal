...
<?php endif; ?>
<section class="max-w-6xl lg:max-w-7xl 2xl:max-w-[1400px] mx-auto px-6 py-24">
  ...
  <?php if ($events): ?>
    ...
    <!-- Two-column layout on lg+: calendar sticks in a left sidebar so the
         right column can breathe wide and expose more per-row detail. -->
    <div class="mt-5 lg:grid lg:grid-cols-[320px_1fr] lg:gap-8 xl:gap-10">
      <!-- Sidebar: calendar + selected-day summary. Stacks above list on
           mobile; sticks below the sticky header on desktop. -->
      <aside class="lg:sticky lg:top-24 self-start"
             x-data="sessionCalendar(<?= htmlspecialchars(json_encode($sessionsByDay), ENT_QUOTES) ?>, <?= htmlspecialchars(json_encode($initialDay), ENT_QUOTES) ?>)" x-cloak>
        ...
        <div class="border border-white/5 rounded-2xl bg-navy-900/40 p-4 sm:p-5 max-w-md lg:max-w-none">
      ...
      <!-- Right column: search + filters, then session list. On lg+ this
           takes the wider half of the split; on mobile it sits below. -->
      <div class="mt-6 lg:mt-0">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between" data-events-toolbar>
        ...
        <div class="mt-6 space-y-3">
          <?php foreach ($events as $event):
            ...
            $rowCover = !empty($event['cover_image']) ? media_src((string) $event['cover_image']) : '';
            $rowAName = trim((string) ($event['package_a_label'] ?? '')) ?: 'Comfort';
            $rowBName = trim((string) ($event['package_b_label'] ?? '')) ?: 'BYO';
            // Short description snippet for the wide desktop layout only —
            // strip tags, collapse whitespace, cap at ~180 chars.
            $rowDescRaw = trim(strip_tags((string) ($event['description'] ?? '')));
            $rowDescRaw = (string) preg_replace('/\s+/', ' ', $rowDescRaw);
            $rowDesc = '';
            if ($rowDescRaw !== '') {
                if (function_exists('mb_strlen') && mb_strlen($rowDescRaw) > 180) {
                    $rowDesc = mb_substr($rowDescRaw, 0, 177) . '…';
                } else {
                    $rowDesc = $rowDescRaw;
                }
            }
          ?>
            <article id="<?= e($cardKey) ?>"
              ...
              <!-- Row layout: on mobile keep the compact 3-column grid;
                   on lg+ add a cover thumbnail column and let the middle
                   column wrap so subtitle + description snippet can breathe. -->
              <div class="grid grid-cols-[auto_1fr_auto] lg:grid-cols-[112px_auto_1fr_auto] gap-3 sm:gap-5 items-center p-4 sm:p-5">

                <!-- Cover thumbnail (lg+ only). Sits at the far left with a
                     small overlay of the date, mirroring cinema-listing
                     visual weight. -->
                <div class="hidden lg:block relative w-28 h-24 rounded-xl overflow-hidden bg-navy-950/60 border border-white/5 shrink-0">
                  <?php if ($rowCover): ?>
                    <img src="<?= e($rowCover) ?>" alt="<?= e($event['title']) ?>" loading="lazy"
                         onerror="this.style.display='none'"
                         class="absolute inset-0 w-full h-full object-cover">
                    <div class="absolute inset-0 bg-gradient-to-t from-navy-950/70 via-navy-950/10 to-transparent"></div>
                  <?php else: ?>
                    <span class="absolute inset-0 flex items-center justify-center font-serif text-4xl text-gold-400/25">◯</span>
                  <?php endif; ?>
                  <span class="absolute bottom-1.5 left-1.5 text-[10px] uppercase tracking-widest text-beige-100/85 bg-navy-950/70 backdrop-blur px-1.5 py-0.5 rounded">
                    <?= e(date('D d M', $startTs)) ?>
                  </span>
                </div>
                ...
                <!-- Title + meta. Truncates on mobile; wraps and shows
                     description + facilitator on lg+ so the wider viewport
                     surfaces the detail the user asked for. -->
                <div class="min-w-0">
                  <h3 class="font-serif text-base sm:text-lg lg:text-xl text-beige-100 leading-tight truncate lg:whitespace-normal lg:overflow-visible"><?= e($event['title']) ?></h3>
                  <?php if (!empty($event['subtitle']) || !empty($event['location'])): ?>
                    <p class="text-xs text-beige-100/55 mt-1 truncate lg:whitespace-normal lg:overflow-visible">
                  ...
                  <?php endif; ?>
                  <?php if ($rowDesc !== ''): ?>
                    <p class="hidden lg:block text-xs text-beige-100/50 mt-2 leading-relaxed line-clamp-2"><?= e($rowDesc) ?></p>
                  <?php endif; ?>
                  <?php if (!empty($event['facilitator'])): ?>
                    <p class="hidden lg:block text-[11px] text-beige-100/45 mt-1.5">
