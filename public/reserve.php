...
// Guest-friendly reservation flow.
//   - Logged-in members: form pre-fills from their profile, standard
//     credits + waiver flow.
//   - Guests: extra name/email/phone block on the form; on submit we
//     find-or-create a lightweight user by email, auto-log them in,
//     then continue into the same booking + payment path as a
//     signed-in member.
$user       = current_user();
$isGuest    = !$user;
$pageTitle  = 'Reserve';
...
$creditBalance = $user ? credit_balance_for((int) $user['id']) : 0;

// Waiver / health disclosure — show the acknowledgement panel if the
// user hasn't accepted yet OR the admin has updated the waiver body
// since their last acceptance. Guests always see it.
$waiverBody      = trim((string) setting('legal_waiver_body', ''));
$waiverUpdatedAt = trim((string) setting('legal_waiver_updated_at', ''));
$acceptedAt      = $user ? trim((string) ($user['waiver_accepted_at'] ?? '')) : '';
...
if (is_post()) {
    ...
    // Guest checkout: capture the visitor's identity before anything
    // else. On success we materialise a lightweight user account, log
    // them in for this session, and then fall through to the normal
    // member booking path as if they'd been signed in the whole time.
    if ($isGuest) {
        $guestName  = trim((string) input('full_name', ''));
        $guestEmail = strtolower(trim((string) input('email', '')));
        $guestPhone = trim((string) input('phone', ''));
        if ($guestName === '')                                    { $errors[] = 'Please share your full name.'; }
        if (!filter_var($guestEmail, FILTER_VALIDATE_EMAIL))       { $errors[] = 'Please enter a valid email.'; }
        if ($guestPhone === '')                                    { $errors[] = 'Please share your mobile number so we can reach you on the day.'; }
        // A very light rate-limit so the endpoint can't be abused to
        // farm user rows via random emails.
        if (!$errors && function_exists('throttle') && function_exists('client_ip')) {
            if (!throttle('reserve_guest:' . client_ip(), 8, 600)) {
                $errors[] = 'Too many reservation attempts from this network. Please try again shortly.';
            }
        }
        if (!$errors) {
            try {
                $g = find_or_create_guest_user($guestEmail, $guestName, $guestPhone);
                login_user_by_id((int) $g['id']);
                $user    = current_user();
                $isGuest = false;
                // Refresh derived state that depended on the anonymous
                // path — credit balance and waiver applicability now
                // resolve against the real user row.
                $creditBalance = $user ? credit_balance_for((int) $user['id']) : 0;
...
<section class="max-w-2xl mx-auto px-6 py-16">
  ...
  <form id="bookForm" method="post" class="mt-10 space-y-5">
    ...
    <?php if ($isGuest): ?>
      <!-- Guest identity block. Only shown when the visitor isn't
           signed in — a lightweight user is created from these fields
           on submit and auto-signed-in so the rest of the flow (waiver,
           checkout, tickets) works normally. -->
      <div class="rounded-2xl border border-gold-500/25 bg-gold-500/5 p-5 space-y-4">
        <div>
          <p class="text-[10px] uppercase tracking-[0.3em] text-gold-400/80">Your details</p>
          <p class="text-xs text-beige-100/60 mt-1">No account signup — we'll hold your seat with just these three lines.
            <a href="<?= url('/public/login.php?next=' . urlencode((string) $_SERVER['REQUEST_URI'])) ?>" class="text-gold-400 hover:text-gold-300 underline-offset-4 hover:underline">Already have an account? Sign in.</a>
          </p>
        </div>
        <div class="grid sm:grid-cols-2 gap-4">
          <label class="block">
            <span class="text-xs text-beige-100/60">Full name</span>
            <input name="full_name" required maxlength="120"
                   value="<?= e((string) input('full_name', '')) ?>"
                   class="mt-1 w-full rounded-2xl bg-navy-950 border border-white/5 px-4 py-3 focus:border-gold-500/50 focus:outline-none">
          </label>
          <label class="block">
            <span class="text-xs text-beige-100/60">Mobile number</span>
            <input name="phone" required maxlength="40"
                   value="<?= e((string) input('phone', '')) ?>"
                   class="mt-1 w-full rounded-2xl bg-navy-950 border border-white/5 px-4 py-3 focus:border-gold-500/50 focus:outline-none">
          </label>
          <label class="block sm:col-span-2">
            <span class="text-xs text-beige-100/60">Email</span>
            <input name="email" type="email" required maxlength="180"
                   value="<?= e((string) input('email', '')) ?>"
                   class="mt-1 w-full rounded-2xl bg-navy-950 border border-white/5 px-4 py-3 focus:border-gold-500/50 focus:outline-none">
            <span class="mt-1 block text-[11px] text-beige-100/40">We'll send your ticket + a link to claim your account here.</span>
          </label>
        </div>
      </div>
    <?php endif; ?>
